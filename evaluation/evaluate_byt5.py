...
def compute_flip_rate(pred_native, pred_romaji):
    ...
    flips = np.sum(pred_native != pred_romaji)
...
def mcnemar_test(y_true, pred_native, pred_romaji):
    ...
    # d: both wrong
    a = np.sum(correct_native & correct_romaji)
    b = np.sum(correct_native & ~correct_romaji)
    c = np.sum(~correct_native & correct_romaji)
    d = np.sum(~correct_native & ~correct_romaji)
    ...
    return {
        ...
        "both_correct": int(a),
        "native_correct_romaji_wrong": int(b),
        "native_wrong_romaji_correct": int(c),
        "both_wrong": int(d),
...
def main():
    ...
    native_config_path = os.path.join(outputs_dir, 'google_byt5_small_config.json')
    romaji_model_path = os.path.join(outputs_dir, 'google_byt5_small_romaji_best_model.pt')
    romaji_config_path = os.path.join(outputs_dir, 'google_byt5_small_romaji_config.json')
    ...
    f1_native = f1_score(labels, pred_native, average='macro')
    
    print("\nNative Text Results:")
    print(classification_report(labels, pred_native, 
    ...
    f1_romaji = f1_score(labels, pred_romaji, average='macro')
    
    print("\nRomaji Text Results:")
    print(classification_report(labels, pred_romaji, 
                               target_names=['Non-Toxic', 'Toxic']))
    ...
    results = {
        ...
        "num_flips": int(np.sum(pred_native != pred_romaji)),
