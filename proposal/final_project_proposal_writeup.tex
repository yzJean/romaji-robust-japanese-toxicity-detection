...
Our evaluation strategy has two complementary goals: (1) measure script-invariance robustness across all three models, and (2) diagnose whether tokenization is the root cause of romaji performance degradation by comparing tokenized vs tokenization-free architectures.
...
\subsection{Implementation and Evaluation Progress}
...
\item Paired native-romaji Japanese toxicity benchmarks:
  ...
  \item Large-scale: llmjp dataset (3847 samples, binary classification)
  \end{itemize}
...
\textbf{Evaluation Metrics Completed} (from Section~\ref{subsec:script-invariance}):
\begin{itemize}
\item Per-Script Performance: Accuracy, precision, recall, F1 (per-class and macro-averaged) for mDeBERTa and BERT Japanese
\item Script-Invariance Gap: $\Delta F_1$ = -0.12 to -0.13 for tokenized models
\item Error Pattern Analysis: Qualitative analysis of distinct failure modes (over-prediction vs toxic recall degradation)
\end{itemize}

\textbf{Pending Evaluation} (from Section~\ref{subsec:script-invariance} and Section~\ref{subsec:tokenization-comparison}):
\begin{itemize}
\item Label Consistency Rate, statistical significance tests (McNemar's), perturbation robustness
\item Tokenization diagnostics: OOV rates, [UNK] frequency, granularity ratios, MeCab segmentation analysis
\item ByT5 training and all hypothesis testing (H1-H3)
\item Cross-architecture analysis: Cohen's Kappa, error taxonomy (Type A/B/C/D), cost-benefit metrics
\item Efficiency metrics (optional): Inference latency, GPU memory, throughput
\end{itemize}
...
\subsubsection{Hypothesis Testing (Requires Working ByT5)}
\begin{itemize}
\item \textbf{H1 - Performance Gap:} Compare $|\Delta F_1|$ between ByT5 and tokenized models to verify if byte-level processing reduces script-invariance gap
\item \textbf{H2 - Toxic Recall Preservation:} Measure whether ByT5 maintains higher toxic recall on romaji than tokenized models
\item \textbf{H3 - Perturbation Resilience:} Test if ByT5 shows smaller F1 degradation under controlled romanization perturbations
\end{itemize}
