...
def evaluate_on_test_data(model, tokenizer, device, data_path, use_romaji=False):
    ...
    # Load test data
    _, test_texts, _, test_labels = load_data(data_path, use_romaji=use_romaji, test_size=0.2)
    ...
    print(classification_report(test_labels, predictions, target_names=['Non-Toxic', 'Toxic'], zero_division=0))
...
def main():
    ...
    # Evaluation mode
        ...
        eval_results = evaluate_on_test_data(model, tokenizer, device, args.data_path, use_romaji)
