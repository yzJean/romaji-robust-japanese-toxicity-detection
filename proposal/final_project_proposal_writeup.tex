...
\section{Evaluation}
\subsection{Robust Script Invarience}
This section systematically evaluates toxicity classifiers for their ability to maintain high performance and consistent predictions across native-script and romanized (romaji) Japanese text. We use the following analytic procedure:

\subsubsection{Classification Performance}
\begin{itemize}
\item \textbf{F1\_native:} Precision, Recall, and F1-score on the held-out native-script test set (per level and macro-averaged)
\item \textbf{F1\_romaji:} Precision, Recall, and F1-score on the held-out romaji test set (per level and macro-averaged)
\end{itemize}

\subsubsection{Script-Invariance Gap}
\begin{itemize}
    ...
    \item \textbf{Label Consistency Rate:} Percentage of test instances whose predicted label is identical between native and romaji forms (Flip rate $=$ 1 $-$ Consistency).
\end{itemize}

\subsubsection{Statistical Significance}
\begin{itemize}
    \item \textbf{McNemar's Test:} Paired McNemar's test between native and romaji predictions to determine whether the error rate difference is statistically significant.
\end{itemize}

\subsubsection{Robustness to Romanization Variants}
\begin{itemize}
    \item For each common perturbation (long-vowel representation, gemination, numeric leet), calculate:
    \[
...
\subsubsection{Error Analysis on Romanized Inputs}
\begin{itemize}
    \item \textbf{High-$\Delta$F1 Case Review:} Manually review the top 50 samples with the largest native–romaji F1 discrepancy to identify common romanization-related prediction failures.
\end{itemize}

\subsubsection{Efficiency Metrics for Deployment (Optional)}
\begin{itemize}
    \item Inference latency (ms/sample) and throughput (samples/s) on GPU/CPU
